...
function CanvasElement({ id, type, props, isSelected, onSelect, position }) {
  ...
  switch (type) {
      ...
      case ELEMENT_TYPES.BUTTON:
          content = <button className="px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600 text-sm pointer-events-none">{props.label}</button>;
          break;
      default:
          content = null;
  }
...
export default function Canvas({ elements, setElements, selectedElementId, setSelectedElementId }) {
  ...
  const handleDrop = useCallback((e) => {
    ...
    let dropX = e.clientX - canvasBounds.left;
    let dropY = e.clientY - canvasBounds.top;

    if (existingElementId) {
        console.log(`Moving existing element ID: ${existingElementId}`);
        ...
        console.log(`Calculated new position: x=${newX}, y=${newY} (offset: ${offsetX}, ${offsetY})`);
        ...
        console.log(`Dropped new element type: ${elementType}`);
